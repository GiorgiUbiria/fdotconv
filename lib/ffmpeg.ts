...
export class FFmpegConverter {
  ...
  private async detectHardwareAcceleration(): Promise<void> {
    ...
    try {
      ...
      console.warn('Could not detect hardware acceleration, using software only:', error);
      this.hwAccelSupport = { nvenc: false, vaapi: false, qsv: false, videotoolbox: false };
    }
    
    this.hwAccelTested = true;
  }

  private async testHardwareAccel(type: string, encoder: string): Promise<boolean> {
    return new Promise((resolve) => {
      // Create a simple test to see if hardware acceleration works
      const args = ['-f', 'lavfi', '-i', 'testsrc=duration=1:size=320x240:rate=1', '-t', '1'];
  ...
  private async getAvailableEncoders(): Promise<string[]> {
    return new Promise((resolve, reject) => {
      ...
      ffmpeg.on('close', (code) => {
        if (code === 0) {
          const encoders = output.match(/^ V\..... (\w+)/gm)
  ...
  async convert(options: ConversionOptions): Promise<ConversionResult> {
    const { inputPath, outputPath, format, fileType, quality = 'medium', onProgress } = options;
    ...
    try {
      ...
      const args = await this.buildFFmpegArgs(inputPath, outputPath, format, fileType, quality, mediaInfo);
  ...
  private buildImageArgs(args: string[], outputPath: string, format: string, quality: string): string[] {
    // Image conversion settings
    switch (format.toLowerCase()) {
      case 'jpeg':
      case 'jpg':
        args.push('-q:v', quality === 'high' ? '2' : quality === 'medium' ? '5' : '8');
        break;
      case 'png':
        args.push('-compression_level', quality === 'high' ? '1' : quality === 'medium' ? '6' : '9');
        break;
      case 'webp':
        args.push('-quality', quality === 'high' ? '90' : quality === 'medium' ? '75' : '60');
        break;
    }
  ...
  private buildVideoArgs(args: string[], outputPath: string, format: string, quality: string, mediaInfo: any): string[] {
    const audioFormats = ['mp3', 'wav', 'aac', 'ogg'];
    
    if (audioFormats.includes(format)) {
      // Video to audio conversion
      const audioStreams = mediaInfo.streams?.filter((s: any) => s.codec_type === 'audio') || [];
      ...
      switch (format.toLowerCase()) {
        case 'mp4':
          ...
          args.push('-c:v', 'libx264', '-crf', settings.crf, '-preset', settings.preset);
  ...
  private buildAudioArgs(args: string[], outputPath: string, format: string, quality: string): string[] {
    const qualitySettings = {
    ...
    switch (format.toLowerCase()) {
      case 'mp3':
        args.push('-c:a', 'libmp3lame', '-b:a', settings.bitrate, '-q:a', settings.vbrQuality);
        break;
      case 'wav':
