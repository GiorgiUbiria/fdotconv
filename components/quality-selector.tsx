  ...
  return (
    <div className="relative">
      <select
        ...
        value={value}
        onChange={(e) =>
          onChange(e.target.value as 'fast' | 'low' | 'medium' | 'high')
        }
        disabled={disabled}
      >
      ...
      >
        <InfoIcon className="h-3 w-3 cursor-help text-muted-foreground hover:text-primary" />
      </div>
      ...
      {/* Tooltip */}
      {showTooltip && (
        <div className="absolute bottom-full left-0 z-50 mb-2 w-64 rounded-md border border-border bg-popover p-2 text-xs shadow-lg">
          <div className="font-medium text-popover-foreground">
            {qualityOptions.find((opt) => opt.value === showTooltip)?.label}
          </div>
          <div className="mt-1 text-muted-foreground">
            {
              qualityOptions.find((opt) => opt.value === showTooltip)
                ?.description
            }
          </div>
          {/* Tooltip arrow */}
          <div className="absolute left-4 top-full h-0 w-0 border-l-4 border-r-4 border-t-4 border-l-transparent border-r-transparent border-t-border"></div>
        </div>
      )}
    </div>
  );
